...
import { deleteEvent, filterEvents, loadEvents } from '../redux/actions';
                ...
                <table class="w-full">
                    ...
                    <tbody class="bg-white">
                        {events && events.map((event) => (
                            <tr class="text-gray-700 hover:bg-gray-100" key={event.id}>
                                <td onClick={() => history.push(`/edit/${event.id}`)} class="px-4 py-3 border">
                                    {event.title}
                                </td>
                                <td onClick={() => history.push(`/edit/${event.id}`)} class="px-4 py-3 font-semibold border">{event.date}</td>
                                <td onClick={() => history.push(`/edit/${event.id}`)} class="px-4 py-3 border">
                                    {(() => {
                                        if (event.status == 0) {
                                ...
                                <td>
                                    <button
                                        ...
                                        type="button"
                                        class="mt-4 bg-transparent hover:bg-red-500 text-red-700 font-semibold hover:text-white py-2 px-3 border border-red-500 hover:border-transparent rounded"
                                    >
                                        Delete
                                    </button>
                                </td>
