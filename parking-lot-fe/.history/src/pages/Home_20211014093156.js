...
const Home = () => {
    ...
    useEffect(() => { 
        if (state.status != "" || state.title != "") {
            ...
            if (state.status != "") {
                operator = '&';
                query += state.status != "" ? `?status=${state.status}` : "";
            }
            query += state.title != "" ? `${operator}title_like=${state.title}` : "";
            dispatch(filterEvents(query));
        } else {
            dispatch(loadEvents());
        }
    }, [state])
                            ...
                            <tr onClick={() => history.push(`/edit/${event.id}`)} class="text-gray-700 hover:bg-gray-100" key={event.id}>
                                <td class="px-4 py-3 border">
                                    <div class="flex items-center text-sm">
                                        <div>
                                            <p class="text-xs md:text-md font-semibold text-black">{event.title}</p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-4 py-3 text-xs md:text-md font-semibold border">{event.date}</td>
                                <td class="px-4 py-3 text-xs md:text-md border">
                                    {(() => {
                                        if (event.status == 0) {
